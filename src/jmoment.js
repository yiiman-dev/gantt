...
function makeJalali() {
    return objectCreate(jalaali.fn);
}

function jalaali() {
    return makeJalali();
}
jalaali.fn = objectCreate(jalaali.fn);
/*
      Expose functions.
    */
// const module = {
...
/*
      Utility helper functios.
    */

jalaali.div = function(a, b) {
    return ~~(a / b);
};

jalaali.mod = function(a, b) {
    return a - ~~(a / b) * b;
};
...
jalaali.j2d = function(jy, jm, jd) {
    ...
    return (
        jalaali.g2d(r.gy, 3, r.march) +
        (jm - 1) * 31 -
        jalaali.div(jm, 7) * (jm - 7) +
        jd -
        1
    );
};
...
jalaali.d2j = function(jdn) {
    var gy = jalaali.d2g(jdn).gy, // Calculate Gregorian year (gy).
        jy = gy - 621,
        r = jalaali.jalCal(jy),
    ...
    if (k >= 0) {
        if (k <= 185) {
            // The first 6 months.
            jm = 1 + jalaali.div(k, 31);
            jd = jalaali.mod(k, 31) + 1;
            return {
                jy: jy,
                jm: jm,
                jd: jd
    ...
    jm = 7 + jalaali.div(k, 30);
    jd = jalaali.mod(k, 30) + 1;
    return {
        jy: jy,
        jm: jm,
        jd: jd
...
jalaali.g2d = function(gy, gm, gd) {
    var d =
        jalaali.div((gy + jalaali.div(gm - 8, 6) + 100100) * 1461, 4) +
        jalaali.div(153 * jalaali.mod(gm + 9, 12) + 2, 5) +
        gd -
        34840408;
    d =
        d -
        jalaali.div(
            jalaali.div(gy + 100100 + jalaali.div(gm - 8, 6), 100) * 3,
            4
        ) +
        752;
    return d;
...
jalaali.d2g = function(jdn) {
    ...
    j =
        j +
        jalaali.div(jalaali.div(4 * jdn + 183187720, 146097) * 3, 4) * 4 -
        3908;
    i = jalaali.div(jalaali.mod(j, 1461), 4) * 5 + 308;
    gd = jalaali.div(jalaali.mod(i, 153), 5) + 1;
    gm = jalaali.mod(jalaali.div(i, 153), 12) + 1;
    gy = jalaali.div(j, 1461) - 100100 + jalaali.div(8 - gm, 6);
    return {
        gy: gy,
        gm: gm,
        gd: gd
...
jalaali.jalCal = function(jy) {
    ...
    for (i = 1; i < bl; i += 1) {
        ...
        leapJ =
            leapJ +
            jalaali.div(jump, 33) * 8 +
            jalaali.div(jalaali.mod(jump, 33), 4);
        jp = jm;
    }
    n = jy - jp;
    ...
    leapJ =
        leapJ + jalaali.div(n, 33) * 8 + jalaali.div(jalaali.mod(n, 33) + 3, 4);
    if (jalaali.mod(jump, 33) === 4 && jump - n === 4) leapJ += 1;

    // And the same in the Gregorian calendar (until the year gy).
    leapG =
        jalaali.div(gy, 4) -
        jalaali.div((jalaali.div(gy, 100) + 1) * 3, 4) -
        150;
    ...
    if (jump - n < 6) n = n - jump + jalaali.div(jump + 4, 33) * 33;
    leap = jalaali.mod(jalaali.mod(n + 1, 33) - 1, 4);
    if (leap === -1) {
        leap = 4;
    }
...
/*
    ...
    */
jalaali.toJalaali = function(gy, gm, gd) {
...
/*
    ...
    */
jalaali.toGregorian = function(jy, jm, jd) {
...
/*
    ...
    */
jalaali.isValidJalaaliDate = function(jy, jm, jd) {
    return (
        jy >= -61 &&
        jy <= 3177 &&
        jm >= 1 &&
        jm <= 12 &&
        jd >= 1 &&
        jd <= jalaali.jalaaliMonthLength(jy, jm)
    );
};
...
/*
    ...
    */
jalaali.isLeapJalaaliYear = function(jy) {
...
/*
    ...
    */
jalaali.jalaaliMonthLength = function(jy, jm) {
    if (jm <= 6) return 31;
    if (jm <= 11) return 30;
...
function dateFromArray(config) {
    ...
    g = jalaali.toGregorian(jy, jm, jd);
    j = jalaali.toJalaali(g.gy, g.gm, g.gd);
...
jMoment.fn.jYear = function(input) {
    ...
    if (typeof input === 'number') {
        j = jalaali.toJalaali(this.year(), this.month(), this.date());
        ...
        return jalaali.toJalaali(this.year(), this.month(), this.date()).jy;
    }
};
...
jMoment.fn.jMonth = function(input) {
    ...
    if (input != null) {
        ...
        return jalaali.toJalaali(this.year(), this.month(), this.date()).jm;
    }
};
...
jMoment.fn.jDate = function(input) {
    ...
    if (typeof input === 'number') {
        j = jalaali.toJalaali(this.year(), this.month(), this.date());
        g = jalaali.toGregorian(j.jy, j.jm, input);
        setDate(this, g.gy, g.gm, g.gd);
        moment.updateOffset(this);
        return this;
    } else {
        return jalaali.toJalaali(this.year(), this.month(), this.date()).jd;
    }
};
